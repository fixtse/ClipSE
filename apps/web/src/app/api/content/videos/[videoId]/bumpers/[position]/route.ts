...
function resolveBumper(
	...
	if (position === "intro") {
		return {
			storageKey: video.introStorageKey,
			mimeType: video.introMimeType ?? "video/mp4",
		};
	}

	if (position === "outro") {
		return {
			storageKey: video.outroStorageKey,
			mimeType: video.outroMimeType ?? "video/mp4",
		};
	}
...
export async function GET(request: Request, context: RouteContext) {
	...
	const { videoId, position } = await context.params;
	const video = await contentVideoRepository.findById(videoId);
	const bumper = video ? resolveBumper(video, position) : null;
